...
async function saveMomentLocation(id, lat, lng, name) {
  ...
  await fetch(SB_URL+'/rest/v1/moments?id=eq.'+id, {
    ...
    body:JSON.stringify({lat:lat, lng:lng, location_name:name})
  });
  loadTRAJ();
}
...
function promptLocation(id, title) {
  ...
  ov.innerHTML = '<div style="background:#1c1c2e;border-radius:16px;padding:24px;width:320px;color:#fff">'
    ...
    + '<input id="_locName" placeholder="地点名称（如：Auckland CBD）" style="width:100%;box-sizing:border-box;padding:10px;border-radius:10px;border:1px solid #333;background:#111;color:#fff;font-size:14px;margin-bottom:10px"/>'
  ...
  document.getElementById('_saveBtn').onclick = async function() {
    ...
    return '<div class="tl-group-label" style="margin-top:4px">'+g.label+'</div>'
      + g.items.map(function(t) {
        return '<div class="tl-row">'
          ...
          + '<div class="tl-bubble'+(t.gold?' gold':'')+'"><span class="mo">'+t.mon+'</span><span class="dy">'+t.day+'</span></div>'
          + '<div class="tl-vline"></div></div>'
