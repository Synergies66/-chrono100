...
async function loadTRAJ() {
  const stored = localStorage.getItem('sb-flerfkkbzrnxreszxmcd-auth-token');
  let token = SB_KEY, userId = null;
  if (stored) {
    try { const p=JSON.parse(stored); token=p?.access_token||SB_KEY; userId=p?.user?.id||null; } catch(e){}
  }
  ...
  } catch(err) { console.error('loadTRAJ:',err); TRAJ=[]; }
  renderLifeViews();
}
...
async function saveMomentLocation(id, lat, lng, name) {
  const stored = localStorage.getItem('sb-flerfkkbzrnxreszxmcd-auth-token');
  let token = SB_KEY;
  try { token=JSON.parse(stored)?.access_token||SB_KEY; } catch(e){}
...
function promptLocation(id, title) {
  ...
  ov.innerHTML=`
    <div style="background:#1c1c2e;border-radius:16px;padding:24px;width:320px;color:#fff">
      ...
      <input id="_locName" placeholder="地点名称（如：Auckland CBD）"
        style="width:100%;box-sizing:border-box;padding:10px;border-radius:10px;
               border:1px solid #333;background:#111;color:#fff;font-size:14px;margin-bottom:10px"/>
      <button id="_gpsBtn" style="width:100%;padding:10px;border-radius:10px;border:none;
        background:#6c63ff;color:#fff;font-size:14px;cursor:pointer;margin-bottom:8px">🌐 使用当前位置</button>
      <button id="_saveBtn" style="width:100%;padding:10px;border-radius:10px;border:none;
        background:#3a3a5c;color:#fff;font-size:14px;cursor:pointer;margin-bottom:8px">✅ 保存</button>
      <button id="_skipBtn" style="width:100%;padding:10px;border-radius:10px;border:none;
        background:transparent;color:#555;font-size:13px;cursor:pointer">跳过</button>
    </div>`;
  document.body.appendChild(ov);
  let _lat=null,_lng=null;
  document.getElementById('_gpsBtn').onclick=()=>
    navigator.geolocation.getCurrentPosition(p=>{
      _lat=p.coords.latitude; _lng=p.coords.longitude;
      const ni=document.getElementById('_locName');
      if(!ni.value) ni.value=`${_lat.toFixed(4)}, ${_lng.toFixed(4)}`;
    },()=>alert('无法获取GPS，请手动输入'));
  document.getElementById('_saveBtn').onclick=async()=>{
    const name=document.getElementById('_locName').value.trim();
    if(!name){alert('请输入地点名称');return;}
    ov.remove();
    await saveMomentLocation(id,_lat||0,_lng||0,name);
  };
  document.getElementById('_skipBtn').onclick=()=>ov.remove();
}

function renderLifeViews() {
  const tlEl = document.getElementById('timelineItems');
  if (!tlEl) return;
  if (!TRAJ.length) {
  ...
  const groups=[
    {label:'Today · '+now.getFullYear(), items:TRAJ.filter(t=>t._date.toDateString()===todayStr)},
    {label:'Last 7 Days', items:TRAJ.filter(t=>t._date<now&&t._date>=week)},
    {label:'Earlier',     items:TRAJ.filter(t=>t._date<week)},
  ].filter(g=>g.items.length);
  tlEl.innerHTML=groups.map(g=>
    `<div class="tl-group-label" style="margin-top:4px">${g.label}</div>`+
    g.items.map(t=>`
      <div class="tl-row">
        ...
        <div class="tl-content" style="margin-left:12px;padding-bottom:10px">
          <div class="tl-card"><div class="tl-card-inner">
            ...
            <div class="tl-card-meta">${t.time} · ${t.loc}
              ${t.needsLoc?`<span onclick="promptLocation('${t.id}','${t.title.replace(/'/g,"&#39;")}')"
                style="color:#6c63ff;cursor:pointer;font-size:11px;margin-left:4px">＋设置位置</span>`:''}
            </div>
          </div></div>
        </div>
      </div>`).join('')
